"""
All fortran subroutines found in this file or any other file within this codebase are taken from https://github.com/nasa/legacy-mars-global-climate-model which describes how the NASA Ames Mars Global Climate Model is used.
"""
...
"""
ExecutableBot.py: Overwrites microphys.f to modify the effective dust radius variable,
    recompiles, renames and moves the compiled binary to a new folder.
"""

# Prefix for executable files and save folder.
# By default, this will save the binaries to /GCM_path/executable_reff_MY24/
save_postfix = "reff_MY24"

# Change this to your Nasa GCM installation absolute path
GCM_path = "/home/user/GCM/"

# Range of executables to create: range_start and range_end are inclusive.
# If setting range_start = 0.5, range_end = 5., step_size = 0.05,
# the binaries 0.5, 0.55, ..., 4.95, 5.00 will be created.
range_start = 0.5
range_end = 5.
step_size = 0.05


def modify_compiled_variable(f):
    return \
f"""      subroutine microphys(dt,pl,tl,kd,sx,sy,Fs,p_pbl,tsurf,co2ice,
...
if __name__ == "__main__":
    for i in np.arange(range_start, range_end + step_size, step_size):
        executable_name = "%.2fe-6" % i
        print("Creating", executable_name)
        cp = subprocess.Popen(["make", "clean"], stderr=subprocess.STDOUT)
        cp.wait()
        with open("./microphys.f", 'w') as w:
            w.write(modify_compiled_variable(executable_name))
        cp = subprocess.Popen(["make"])
        cp.wait()
        os.makedirs(f"{GCM_path}{'/' if GCM_path[-1] is not '/' else ''}executable_{save_postfix}/", exist_ok=True)
        shutil.move("./gcm2.3", f"{GCM_path}{'/' if GCM_path[-1] is not '/' else ''}executable_{save_postfix}/{executable_name}")
